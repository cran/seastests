...
if(freq <= 1) {warning("Frequency is <= 1, this series does not seem to be seasonal. Results will almost surely be misguiding.")}
...
.sim_int <- function(Length, Freq) {
  ...
  y <- utils::tail(stats::diffinv(y1, lag=Freq, xi=stats::rnorm(Freq,0,10)), Length) # seasonal diffinverting y1 to get a regular+seasonally intgr. series
  y <- stats::ts(100+3*scale(y)*(stats::rbinom(1, 1,0.5)*2-1), frequency=Freq, start=c(2001,1))
  return(y)
}
